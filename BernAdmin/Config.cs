...
namespace LambAdmin
{
    public partial class DGAdmin
    {
        ...
        public void CFG_Dynprop_Apply()
        {
            ...
            if (ConfigValues.settings_ingametitle)
        ...
        private void patchGame()
        {
            ...
            //Gametype setup
            //if (ConfigValues.settings_servertitle) AfterDelay(1000, () => memoryScanning.writeToServerInfoString(0x00400000, 0x10000000));
            if (ConfigValues.settings_servertitle && ConfigValues.settings_ingametitle) AfterDelay(50, () => memoryScanning.writeGameInfoString());
        }


            private static string generateServerString(string mode, string map)
        {
            //Create our server info string
            string privateClients = GSCFunctions.GetDvar("sv_privateClientsForClients");
            string port = GSCFunctions.GetDvar("net_masterServerPort");
            return @"gn\IW5\gt\" + mode + @"\hc\0\pu\1\m\" + map + @"\px\-1\pn\\mr\\pc\" + privateClients + @"\ff\0\fg\\md\\kc\1\ac\1\d\2\qp\" + port + @"\vo\1\";
        }
        ...
        public class memoryScanning
        {
            ...
            /*private static void scanServerInfo_Completed(object sender, RunWorkerCompletedEventArgs e)
            {
                if (e.Cancelled)
                {
                    Utilities.PrintToConsole("e.cancelled!!");
                    return;
                }
                if (e.Error != null)
                {
                    Utilities.PrintToConsole("e.error!!!!" + e.Error.Message);
                    return;
                }
                ...
                if (addrs.Count == 0)
                {
                    Utilities.PrintToConsole("no addrs");
                    return;
                }
            ...
            private static void scanGameInfo_Completed(object sender, RunWorkerCompletedEventArgs e)
            {
                if (e.Cancelled)
                {
                    Utilities.PrintToConsole("e.cancelled 2... 1120");
                    return;
                }
                if (e.Error != null)
                {
                    Utilities.PrintToConsole("e.error 2 ... 1125" + e.Error.Message);
                    return;
                }
                ...
                if (addrs.Count == 0)
                {
                    Utilities.PrintToConsole("no addrs 2 . 1132");
                    return;
                }
            ...
            /*private static void setServerInfoPtrs(List<IntPtr> addrs)
            {
                ...
                {
                    Utilities.PrintToConsole("setserverinfoptrs no addrs! 1121");
                    return;
                }
            }*/
        ...
        private static void writeGameInfo()
        {
            ...
            foreach (int ptr in gameInfo)
            {
                ...
                try
                {
                    ...
                    //WriteLog.Debug(infoText);
                    Regex rgx = new Regex("g_gametype\\\\.*?\\\\");
                    string matchedValue = rgx.Match(infoText).Value;
                    if (!matchedValue.Contains(ConfigValues.servertitle_mode)) {
                        infoTextBackup = matchedValue;
                    }
                    infoText = rgx.Replace(infoText, "g_gametype\\" + ConfigValues.servertitle_mode + "\\");
                    //byte[] infoBytes = Encoding.ASCII.GetBytes(infoText);
                    memoryScanning.Mem.WriteString(new IntPtr(ptr), infoText);
                    //WriteLog.Debug("success 1161");
                    //WriteLog.Debug(infoText);
                }
                catch
                {
                    WriteLog.Debug("fuk 1161");
                    break;
                }
            }
        }
        ...
        {
            if (!ConfigValues.settings_servertitle) return;
            ...
            foreach (int ptr in gameInfo)
            {
                ...
                try
                {
                    ...
                    //WriteLog.Debug(infoText);
                    Regex rgx = new Regex("g_gametype\\\\.*?\\\\");
                    infoText = rgx.Replace(infoText, infoTextBackup);
                    //byte[] infoBytes = Encoding.ASCII.GetBytes(infoText);
                    memoryScanning.Mem.WriteString(new IntPtr(ptr), infoText);
                }
                catch
                {
                    WriteLog.Debug("shit 1183");
                }
            }
        }
